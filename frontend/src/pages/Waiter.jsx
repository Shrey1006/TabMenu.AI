...
function WaiterBoard() {
  ...
  const alertColor = {
    dish_ready: "border-green-400 bg-green-50",
    service_request: "border-warm-500 bg-warm-50",
    payment: "border-blue-400 bg-blue-50",
  };
      ...
      <div className="mx-auto max-w-5xl p-6">
        ...
        <div className="space-y-3">
          {alerts.map((a) => (
            <div
              key={a.id}
              className={`flex items-center justify-between rounded-xl border-l-4 p-4 ${alertColor[a.type] || "bg-white"}`}
            >
              <div className="flex items-center gap-3">
          ...
          {alerts.length === 0 && (
            <p className="rounded-xl bg-white p-6 text-center text-stone-400">
              No alerts — all tables serviced
            </p>
          )}
        </div>

        <h2 className="mb-4 mt-8 text-lg font-bold text-stone-800">
          Ready for Dispatch
        </h2>
